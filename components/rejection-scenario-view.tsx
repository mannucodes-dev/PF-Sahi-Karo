...
import { RemarkCodeKey, MOCK_REMARK_CODES } from "@/lib/mock-data";
...
interface RejectionScenarioViewProps {
  claimId: string;
  defaultCode: RemarkCodeKey;
}

const SCENARIOS: { code: RemarkCodeKey; label: string; short: string }[] = [
  { code: "NAME_MISMATCH", label: "Aadhaar Name Mismatch (Primary Demo)", short: "Name Mismatch" },
...
export function RejectionScenarioView({
  ...
  const [selectedCode, setSelectedCode] = useState<RemarkCodeKey>(defaultCode);
  const activeRemark = getDecoderResult(selectedCode) || MOCK_REMARK_CODES.NAME_MISMATCH;

  return (
    <div className="space-y-6">
      {/* Scenario Switcher Bar for Judges / Evaluators */}
      <div className="bg-slate-100/90 border border-slate-200 rounded-xl p-3.5 sm:p-4 space-y-2.5 shadow-2xs">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 text-xs font-bold text-zinc-700">
            ...
            <span>Interactive Demo: Test All 5 Rejection Scenarios</span>
          </div>
          <span className="text-[11px] font-mono text-zinc-500 bg-white border border-slate-200 px-2 py-0.5 rounded">
            Active: {selectedCode}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {SCENARIOS.map((scenario) => {
            ...
            return (
              ...
              >
                {scenario.short}
                {scenario.code === "NAME_MISMATCH" && (
                  <span className="ml-1 text-[10px] opacity-85">★ Demo</span>
                )}
              </button>
            );
          })}
