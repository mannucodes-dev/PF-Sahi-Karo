...
export default async function ClaimResubmitPage({
  params,
}: {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col items-center justify-center p-6 text-zinc-900">
      <Card className="w-full max-w-lg border-zinc-200">
        <CardHeader>
          <CardTitle className="text-xl font-bold">Guided Resubmit: {id}</CardTitle>
          <CardDescription>
            Session 1 Scaffold Placeholder — Guided fix & confirmation flow in Session 2.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="rounded-lg border border-dashed border-zinc-300 p-4 text-center text-sm text-zinc-500">
            Resubmission document correction flow will render here.
          </div>
          <Link
            href="/"
            className={cn(
